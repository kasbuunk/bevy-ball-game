...
pub fn update_enemy_direction(
    ...
    let x_min = 0.1 + half_sprite_size;
    let x_max = window.width() - half_sprite_size - 0.1;
    let y_min = 0.1 + half_sprite_size;
    let y_max = window.height() - half_sprite_size - 0.1;
    ...
    for (transform, mut enemy) in enemy_query.iter_mut() {
        ...
        if direction_changed {
            let sound_effect = "audio/pluck_001.ogg";
...
pub fn handle_game_over(mut game_over_event_reader: EventReader<GameOver>) {
    match game_over_event_reader.read().next() {
        Some(event) => println!("Game over! Score: {}", event.score),
        None => (),
    };
}
